...
class Menu(QWidget):
    ...
    def _build_changelog_tab(self):
        ...
        txt.setPlainText(
            ...
            "\n"
            "[Image Paint]\n"
            "  + Fixed crash when applying image to character.  The native\n"
            "    HitTestAtScreenPosition worker (used for UV calibration) has a\n"
            "    different ABI than assumed — it dereferences its 6th argument\n"
            "    (passed as null) at offset +0x38, causing EXCEPTION_ACCESS_\n"
            "    VIOLATION reading 0x38 before a single stamp is sent.  UV\n"
            "    calibration now uses a safe geometry-only path (body AABB +\n"
            "    atlas border constants) with no native calls — no more crash.\n"
            "\n"
            "[Paint Presets]\n"
