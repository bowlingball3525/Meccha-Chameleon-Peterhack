...
class Menu(QWidget):
    ...
    def _build_changelog_tab(self):
        ...
        txt.setPlainText(
            ...
            "  + Centered mode now uses PaintAtScreenPosition (the same raycast\n"
            "    system as camouflage) for the front face.  The game maps each\n"
            "    screen pixel to the correct UV position automatically — no\n"
            "    UV-atlas guessing required.  The image is always perfectly\n"
            "    centred on the visible body, regardless of paint-sphere layout.\n"
            "  + Back face is painted via UV at the spine centre (u=0.75) using\n"
            "    fixed extents — still no hit-test calls, no crash risk.\n"
            "  + Projector mode unchanged: image stretches across the full UV\n"
            "    atlas (u 0→1) for a continuous front-to-back wrap.\n"
            "  + Fixed crash (EXCEPTION_ACCESS_VIOLATION reading 0x38): the\n"
            "    native HitTestAtScreenPosition worker dereferences its 6th arg\n"
            "    at +0x38 when passed null — game crash before any stamp was\n"
            "    sent.  All hit-test calls are now removed from the paint path.\n"
            "\n"
            "[Paint Presets]\n"
