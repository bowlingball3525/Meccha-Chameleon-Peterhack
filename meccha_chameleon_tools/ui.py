...
class Menu(QWidget):
    ...
    def _build_changelog_tab(self):
        ...
        txt.setPlainText(
            ...
            "\n"
            "[Image Paint]\n"
            "  + Fixed crash when applying image to character.  The hit-test\n"
            "    shellcode (used for UV calibration) allocated only 0x20 bytes of\n"
            "    stack (shadow-space only) but then wrote the PlayerController arg\n"
            "    to [rsp+0x20] — exactly where the saved RBP lives.  This corrupted\n"
            "    the remote thread's stack frame and crashed the game on every paint.\n"
            "    Fix: changed 'sub rsp, 0x20' to 'sub rsp, 0x40' (matching the\n"
            "    PaintAtScreenPosition shellcode which already had 0x60).\n"
            "\n"
            "[Paint Presets]\n"
