...
def vk_from_name(name):
    ...
    if len(key) == 1 and key.isalpha():
        key = key.upper()
    return KEY_VK.get(key, 0x2D)  # default Insert
...
def menu_text_input_focused():
    """True when keyboard focus is in a Peterhack text field (rename, etc.)."""
    try:
...
# ---------------------------------------------------------------------------
class KeyRecorder:
    def __init__(self, on_record, parent=None):
...
class Menu(QWidget):
    ...
    def __init__(self, config: Config, esp: MecchaESP):
        ...
        if hasattr(self.esp, "set_rename_notify"):
            self.esp.set_rename_notify(
                lambda ok, msg: self.exploits_rename_finished.emit(ok, msg or "")
            )
        self._peter_logo_poll_timer = QTimer(self)
        self._peter_logo_poll_timer.timeout.connect(self._poll_peter_logo_ready)
    ...
    def _on_bridge_preload_done(self, ok):
        ...
        if ok and getattr(self.config, "exploits_anti_kick", False):
    ...
    def _camo_menu_worker(self):
        ...
        try:
            ...
            ok = self.esp.camo_apply(full_wrap=True)
    ...
    def _run_exploits_rename(self):
        if getattr(self, "_exploits_rename_cooldown_active", False):
            return
        name = self.txt_exploits_rename.text().strip()[:32]
        if not name:
            self.lbl_exploits_rename_status.setText("Enter a name first.")
            return
        self._exploits_rename_cooldown_active = True
        self.btn_exploits_rename.setEnabled(False)
        QTimer.singleShot(1000, self._exploits_rename_cooldown_done)
        self.config.exploits_rename_text = name
        if hasattr(self.esp, "queue_exploits_rename"):
            self.esp.queue_exploits_rename(name, self.config, force=True)
            self.lbl_exploits_rename_status.setText(f"Queued '{name}'...")
        else:
            self.lbl_exploits_rename_status.setText("Rename unavailable — restart Peterhack.")

    def _exploits_rename_cooldown_done(self):
        self._exploits_rename_cooldown_active = False
        self.btn_exploits_rename.setEnabled(True)
    ...
    def _return_to_lobby_worker(self):
        ...
        try:
            ...
            msg = str(exc)
        self.return_lobby_finished.emit(ok, msg or "")
    ...
    def _exploits_rename_done(self, ok, msg):
        short = msg if len(msg) <= 140 else msg[:137] + "..."
        if ok:
            self.lbl_exploits_rename_status.setText("OK — " + short)
        else:
            self.lbl_exploits_rename_status.setText("Failed — " + short)
    ...
    def _stop_camo_now(self):
        ...
        if getattr(self, "_stop_thread", None) and self._stop_thread.is_alive():
            return
    ...
    def _stop_camo_worker(self):
        ...
        self.stop_camo_finished.emit(bool(ok))

    def _stop_camo_done(self, ok):
        self._camo_busy = False
        self.lbl_camo_status.setText("Stopped" if ok else "Stop failed")
        self._camo_set_overlay_feedback("CAMO STOPPED" if ok else "STOP FAIL", 60)
        self._update_camo_buttons()
        QTimer.singleShot(
            2000,
            lambda: self.lbl_camo_status.setText("Ready — click Paint Now"),
    ...
    def _register_global_hotkeys(self):
        ...
        # F10/F9 are polled each frame — no external controller needed.
    ...
    def _register_magnet_hotkey(self):
        """Magnet uses GetAsyncKeyState polling (not RegisterHotKey — Qt eats plain keys)."""
    ...
    def _magnet_hotkey_allowed(self):
        """Allow magnet in-game or while any Peterhack window is focused (not while typing)."""
        if menu_text_input_focused():
            return False
        if is_game_foreground(self._game_hwnd_for_input()):
            return True
        try:
            import os
            user32 = ctypes.windll.user32
            ...
            pid_fg = ctypes.c_ulong()
            user32.GetWindowThreadProcessId(int(fg), ctypes.byref(pid_fg))
            return pid_fg.value != 0 and pid_fg.value == os.getpid()
        except Exception:
            return False

    def _try_magnet_key_toggle(self):
        """Edge-triggered magnet toggle from physical key state (works outside Qt text input)."""
        if not self._magnet_hotkey_allowed():
        ...
        if down and not was_down and hasattr(self.esp, "toggle_magnet"):
    ...
    def _start_peter_logo_fetch(self):
        ...
        self._peter_logo_fetch_result = [None]
        self._peter_logo_fetch_done = _threading.Event()
        done = self._peter_logo_fetch_done
        result = self._peter_logo_fetch_result
        ...
        self._peter_logo_poll_timer.start(200)

    def _poll_peter_logo_ready(self):
        done = getattr(self, "_peter_logo_fetch_done", None)
        if done is None or not done.is_set():
            return
        self._peter_logo_poll_timer.stop()
        data = (getattr(self, "_peter_logo_fetch_result", None) or [None])[0]
        self._peter_logo_fetch_done = None
        self._peter_logo_fetch_result = None
        if not data:
            return
    ...
    def _open_discord(self):
        webbrowser.open("https://discord.gg/7T3damu79F")
    ...
    def _build_ui(self):
        ...
        self.btn_discord.setStyleSheet(
            f"QPushButton {{ background-color: {self.DISCORD_BLURPLE}; color: #ffffff;"
            f" border: 1px solid {self.DISCORD_BLURPLE_HOVER}; font-weight: bold; }}"
            f" QPushButton:hover {{ background-color: {self.DISCORD_BLURPLE_HOVER}; }}"
        ...
        bar.addWidget(self.btn_discord)
        bar.addStretch()
        bar.addWidget(hint)
        outer.addLayout(bar)
    ...
    def _build_exploits_tab(self):
        ...
        lo.addWidget(self._chk("No Decoy Cooldown", "exploits_no_decoy_cooldown"))
        cb_anti_detect = self._chk("Anti Detection (Survivor)", "exploits_anti_detection")
        ...
        lo.addWidget(self._chk("Infinite Bullets (Hunter)", "exploits_infinite_bullets"))
        ...
        self.spn_decoy_count.valueChanged.connect(
            lambda v: setattr(self.config, "exploits_decoy_count", int(v))
        )
        ...
        cb_anti_kick.setToolTip(
            "Blocks host kick RPCs on your PlayerController (ClientWasKicked, return-to-menu, etc.).\n"
            "If enabled before a match: waits until you spawn in + 5s stable before hooking (prevents crashes).\n"
            "Safest: join match first, spawn in, then toggle Anti-Kick on.\n"
            "Blocked kicks logged to C:\\peterhack\\logs\\anti_kick.log.\n"
            "EOS/platform session kicks may still disconnect you."
        )
        ...
        self.btn_exploits_rename.clicked.connect(self._run_exploits_rename)
        rr.addWidget(self.btn_exploits_rename)
        lo.addLayout(rr)
        self.lbl_exploits_rename_status = QLabel("")
        self.lbl_exploits_rename_status.setWordWrap(True)
        self.lbl_exploits_rename_status.setStyleSheet("color: #6a8a5a; font-size: 10px;")
        lo.addWidget(self.lbl_exploits_rename_status)
    ...
    def _build_misc_tab(self):
        ...
        self.sld_ui_overlay_fps.blockSignals(True)
        self.sld_ui_overlay_fps.setValue(init_fps)
        self.sld_ui_overlay_fps.blockSignals(False)
        self.config.ui_overlay_fps = init_fps
        self.sld_ui_overlay_fps.setTickPosition(QSlider.TicksBelow)
        self.sld_ui_overlay_fps.setTickInterval(30)
        self.sld_ui_overlay_fps.setToolTip(
            "Overlay repaint rate (1–100 FPS). Release mouse to apply."
        )
        self.sld_ui_overlay_fps.valueChanged.connect(self._on_ui_overlay_fps_label)
        self.sld_ui_overlay_fps.sliderReleased.connect(self._on_ui_overlay_fps_apply)
        fps_row.addWidget(self.sld_ui_overlay_fps)
        self.lbl_ui_overlay_fps = QLabel(f"{init_fps} FPS")
        self.lbl_ui_overlay_fps.setStyleSheet(
            "color: #8ab870; font-size: 10px; min-width: 56px;"
        )
    ...
    def _build_changelog_tab(self):
        ...
        txt.setPlainText(
            ...
            "\n"
            "[Stability]\n"
            "  + Exploits loop off UI thread; async anti-kick bridge sync.\n"
            ...
            "    exploits polls get_anti_kick_log over bridge TCP.\n"
            ...
            "  + Front pass noclip held through entire paint (exploits tick fix).\n"
            ...
            "  + Rotate moves camera view only (ControlRotation) — body stays put.\n"
            "  + Bridge rotate + ProcessEvent fallback when DLL lacks rotate cmd.\n"
            "  + Updated meccha-xenos-bridge.dll bundled with Peterhack.\n"
            "\n"
            "[PLAYERS tab]\n"
            "  + Session player list with Steam64 IDs.\n"
            "  + Copy Steam ID, save to blocklist, optional autokick.\n"
            "  + Blocklist at C:\\peterhack\\blocked_players.json.\n"
            "\n"
            ...
            "  + Debug logging to latest.log ([EXPLOITS:TAG] lines).\n"
            "\n"
    ...
    def _build_debugging_tab(self):
        ...
        log_specs = [
            ...
            ("Exploits ([EXPLOITS:*])", "log_exploits"),
            ("Anti-kick & freecam ([EXPLOITS:ANTI-KICK])", "log_anti_kick"),
...
class Overlay(QWidget):
    ...
    def __init__(self, esp: MecchaESP, config: Config, menu=None):
        ...
        self.esp.start_exploits_loop(lambda: self.config)
        # Defer heavy ESP reads so the menu can paint/respond first.
        QTimer.singleShot(800, lambda: self.esp.start_esp_snapshot_loop(lambda: self.config))
    ...
    def _esp_overlay_active(self) -> bool:
        """Any feature that drives snapshot / paint work."""
        c = self.config
        return bool(
            getattr(c, "enabled", False)
            or getattr(c, "aimbot_enabled", False)
            or getattr(c, "skeleton_esp", False)
            or getattr(c, "clone_esp", False)
            or getattr(c, "radar_enabled", False)
        )
    ...
    def _sync_overlay_timer_interval(self):
        ...
        if prev_ms == ms and timer is not None and timer.isActive():
            return
        self._overlay_interval_applied_ms = ms
        if timer is None:
            return
        try:
        ...
        elif self._esp_overlay_active() and effective < target:
            print(
                f"[UI] overlay timer {effective} FPS ({ms} ms) — ESP active (saved {target} FPS)",
                flush=True,
            )
        elif prev_ms != ms:
    ...
    def set_ui_overlay_fps(self, fps: int):
        self.request_set_fps(fps)

    def apply_overlay_fps(self):
        self.request_set_fps(getattr(self.config, "ui_overlay_fps", 60))

    def _set_menu_ui_active(self, active: bool):
        """Track menu visibility; throttle overlay + ESP snapshot while menu is open."""
        self.esp._menu_ui_active = bool(active)
        self._sync_overlay_timer_interval()
    ...
    def update_overlay(self):
        ...
        if menu is not None:
            ...
            if visible != self._last_menu_visible:
                ...
                self.esp._menu_ui_active = visible
                self._sync_overlay_timer_interval()
        esp_active = self._esp_overlay_active()
    ...
    def _poll_keys(self):
        ...
        if menu and hasattr(menu, "_try_magnet_key_toggle"):
            menu._try_magnet_key_toggle()
    ...
    def _sample_screenspace_pattern(self, full_body=False):
        ...
        # Ring sampling projects outside the body bbox — always capture full viewport.
        rx0, ry0 = 0, 0
        rx1, ry1 = sw, sh
        rw, rh = sw, sh
        halo_px_x = halo_px_y = 0
        mode = "atlas_wrap" if full_body else "island_ring_capture"
        ...
        with self.esp.camo_sampling_hide_local(
            local_pawn, enabled=hide_body, settle_ms=450, pump_frame=_pump_render,
        ):
            for _ in range(3):
                _pump_render()
                _time.sleep(0.05)
        ...
        def sample_env_at(lx, ly):
            """Read environment colour; walk outward if the body is still visible."""
            lx_i, ly_i = int(lx), int(ly)
            in_body = (
                ib_x0 <= lx_i <= ib_x1
                and ib_y0 <= ly_i <= ib_y1
            )
            if hide_body_capture and not in_body:
                c = sample_block(lx_i, ly_i)
                if c and self._camo_pixel_ok(c):
                    return c
            elif hide_body_capture and in_body:
                c = sample_block(lx_i, ly_i)
                if c and not self._is_likely_player_model_pixel(c):
                    return c
            return nearest_bg(lx_i, ly_i)
        ...
        if not full_body:
            # Ring environment colours projected through paint-sphere UV math,
            # stamped only on calibrated front-body islands (stable frozen PaintAtUV).
            ...
            for u0, v0, u1, v1, _iy0, _iy1 in MecchaESP.PAINT_FRONT_ISLANDS:
                fill_uv_rect(
                    u0 + border, v0 + border, u1 - border, v1 - border,
                    island_seg, island_seg,
                )

            fu0, fv0, fu1, fv1 = MecchaESP.PAINT_FRONT_HEMI
            fill_uv_rect(
                fu0 + border, fv0 + border, seam - border, fv1 - border,
                seg_u=seg, seg_v=seg,
            )
            ...
            print(
                f"[CAMO-SS] island_ring_proj points={len(points)} front={front_n} "
                f"back={back_n} unique_colors={unique_cols} grid={seg} "
                f"ring={ring_dist:.0f} quality={quality} bg={median_bg} "
                f"direct={hide_body_capture} mode=island_ring_proj",
                flush=True,
            )
            if len(points) < 4:
                print("[CAMO-SS] too few island_ring_proj points", flush=True)
                return None
            return {"kind": "uv", "points": points}

        front_u = MecchaESP.PAINT_FRONT_U
        pitch_abs = abs(rot[0]) if rot else 0.0
        prone = pitch_abs > 45.0
        ring_dist = 155.0 if (full_body and prone) else 120.0
        ...
        print(
            f"[CAMO-SS] front hemi grid {seg}×{seg} "
            f"u∈[{fu0 + border:.2f},{seam - border:.2f}] ring={ring_dist:.0f}",
            flush=True,
        )
        ...
        if full_body:
            ...
            edge_us = (0.01, seam - 0.02, seam + 0.02, 0.99)
        else:
            edge_us = (0.01, seam - 0.02)
        ...
        for edge in edge_us:
            for frac in [(k + 0.5) / seg for k in range(seg)]:
                ...
                if full_body or (edge < seam and frac < seam - border):
        ...
        print(f"[CAMO-SS] points={len(points)} front={front_n} back={back_n} "
              ...
              f"direct={hide_body_capture} mode={mode} ring={ring_dist:.0f}", flush=True)
        if len(points) < 4:
    ...
    def _paint_esp(self, event):
        ...
        # Camera comes from the snapshot thread only — never block paint on get_camera().
        ...
        if self._paint_throttle:
            ...
            if (
                ...
                cap_note = f" esp-cap:{self.ESP_ACTIVE_OVERLAY_FPS_CAP}{cap_note}"
            _draw_label(
                painter, 10, 40,
    ...
    def _aim_at(self, target_pos, camera=None):
        cam = camera or getattr(self, "_last_cam", None)
        if not cam:
            cam = self.esp.get_camera()
        if not cam:
            return
